...
export function buildFaqs(amount: number, txResult: PaycheckResult): FAQ[] {
  ...
  return [
    ...
    },
    {
      ...
      answer: `On a standard 2,080-hour year, 40 hours a week for 52 weeks, ${formatted} is ${formatCurrencyRound(hourly)} an hour before tax. After federal tax and FICA the effective rate falls to about ${formatCurrencyRound(txResult.netAnnual / 2080)} an hour in a no-income-tax state, or ${formatCurrencyRound(caResult.netAnnual / 2080)} in California. Across 260 working days that is ${formatCurrencyRound(txResult.netAnnual / 260)} a day in hand. Freelance work on top is taxed at your ${formatPercent(txResult.marginalFederalRate * 100, 0)} marginal rate plus self-employment tax, so it has to pay well above the headline hourly figure to be worth the hours.`,
    },
    {
      question: `How much can I save on taxes at ${formatted} by contributing to a 401(k)?`,
      answer: `Your marginal federal rate at ${formatted} is ${formatPercent(txResult.marginalFederalRate * 100, 0)}, so every dollar diverted into a traditional 401(k) saves that much in tax. The full $23,500 allowance for ${TAX_YEAR} cuts the federal bill by roughly ${formatCurrencyRound(savings401k)}, about ${formatCurrencyRound(savings401k / 26)} per bi-weekly cheque. Even a 10 per cent contribution, ${formatCurrencyRound(amount * 0.10)} a year, saves some ${formatCurrencyRound(amount * 0.10 * txResult.marginalFederalRate)} federally, plus state tax where it applies. A 3 per cent employer match adds ${formatCurrencyRound(amount * 0.03)} on top, which is the part no investment return can match.`,
    },
    {
      question: `Which states offer the best take-home pay on ${formatted}?`,
      answer: `The nine states without a personal income tax, among them Texas, Florida, Washington and Nevada, leave the most on ${formatted}. A single filer keeps ${formatCurrencyRound(txResultTX.netAnnual)} in Texas against ${formatCurrencyRound(caResult.netAnnual)} in California and ${formatCurrencyRound(nyResult.netAnnual)} in New York. The Texas-California gap of ${formatCurrencyRound(txVsCaDiff)} is ${formatPercent((txVsCaDiff / amount) * 100, 1)} of gross pay. Flat-tax states sit in between: Pennsylvania takes ${formatCurrencyRound(amount * 0.0307)} at 3.07 per cent. Set against that, no-income-tax states recover the money through property and sales taxes.`,
    },
    {
